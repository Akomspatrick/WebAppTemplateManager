...
namespace DocumentVersionManager.Application.CQRS.DocumentType.Handlers
{
    public  class CreateDocumentTypeCommandHandler  :  IRequestHandler<CreateDocumentTypeCommand, Either<GeneralFailure, Guid>>
    {
        ...
        }var entity =null;// Domain.Entities.DocumentType.Create(request.modelTypeCreateDTO.ModelTypeName, request.modelTypeCreateDTO.Value.GuidId);return ( await _unitOfWork.DocumentTypeRepository.AddAsync(entity, cancellationToken)). Map((x) =>  entity.GuidId);
    }
}