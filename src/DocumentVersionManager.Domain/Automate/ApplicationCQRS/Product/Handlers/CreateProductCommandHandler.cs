...
namespace DocumentVersionManager.Application.CQRS.Product.Handlers
{
    public  class CreateProductCommandHandler  :  IRequestHandler<CreateProductCommand, Either<GeneralFailure, Guid>>
    {
        ...
        }var entity =null;// Domain.Entities.Product.Create(request.modelTypeCreateDTO.Value.ModelTypeName, request.modelTypeCreateDTO.Value.GuidId);return ( await _unitOfWork.ProductRepository.AddAsync(entity, cancellationToken)). Map((x) =>  entity.GuidId);
    }
}